...
func (r *TaskRouter) RegisterRoutes(api *gin.RouterGroup) {
	...
	board := api.Group("/workspaces/:workspace_id/projects/:project_id", auth)
	{
		...
		board.PATCH("/tasks/:task_id/move", r.mw.RequireProjectPermission(middleware.PermTaskMove), r.boardHandler.MoveTask)
	}
	...
	assignees := api.Group("/workspaces/:workspace_id/projects/:project_id/tasks/:task_id/assignees", auth)
	{
		...
		assignees.POST("/", r.mw.RequireProjectPermission(middleware.PermTaskAssign), r.assigneeHandler.AssignMembers)
		assignees.DELETE("/", r.mw.RequireProjectPermission(middleware.PermTaskAssign), r.assigneeHandler.UnassignMembers)
		// Self assign/unassign chỉ cần là member
		assignees.POST("/me", r.mw.RequireProjectMember(), r.assigneeHandler.SelfAssign)
		assignees.DELETE("/me", r.mw.RequireProjectMember(), r.assigneeHandler.SelfUnassign)
	}
